...
class Kite extends RigidBody{
	...
	constructor(){ super();
		...
		this.center_of_gravity_from_front = 0.45;//0.33;//0.33;//fraction of the full chord length
		this.bridle_length = 0.5;//-0.05;//in meters, with respect to the wing join center
		this.tannenbaum_length = 0;
		this.height_of_c_g = 0;//-0.08;// with respect to the c_g of the wing geometry.
		this.elevonData = new Object();
		this.elevonData.dimensions = new THREE.Vector3(0.15, 0.25, 0.005);
		this.elevonData.distanceFromCenter = 0.45;
		this.rudderData = new Object();
		this.rudderData.dimensions = new THREE.Vector3(0.15, 0.2, 0.005);
		this.rudderData.StabilizerDimensions = new THREE.Vector3(0.4, 0.2, 0.005);
		this.rudderData.distanceFromCG = 0.45;
		this.reflexData = new Object();
		this.reflexData.dimensions = new THREE.Vector3(0.10, 0.8, 0.005);
		...
		this.stabilizerLeft.position.z = this.rudderData.StabilizerDimensions.y*0.5;
		this.stabilizerLeft.position.y = this.dimensions.y*0.5;
		...
		this.rudder.position.z = this.rudderData.dimensions.y*0.5;
		...
		this.leftReflex.rotation.y = Math.PI * 0.1;
		this.reflexMeshLeft.position.y = this.dimensions.y * 0.25;
		...
		this.rightReflex.rotation.y = Math.PI * 0.1;
		this.reflexMeshRight.position.y = - this.dimensions.y * 0.25;
		
		// LEFT ELEVON
		this.elevonMeshLeft = new FlatPlate(this.elevonData.dimensions.x, this.elevonData.dimensions.y, this.elevonData.dimensions.z, this, 'teal', false);
		...
		this.leftElevon.rotation.y = Math.PI * 0.1;
		
		// RIGHT ELEVON
		this.elevonMeshRight = new FlatPlate(this.elevonData.dimensions.x, this.elevonData.dimensions.y, this.elevonData.dimensions.z, this, 'teal', false);
		...
		this.rightElevon.rotation.y = Math.PI * 0.1;
		...
		this.leftPropeller = new Propeller(propeller_radius, this);
		...
		this.rightPropeller = new Propeller(propeller_radius, this);
		...
		for (let visiblePart of visibleKiteParts) this.kiteMeshes.add(visiblePart);
	...
	calculateAndSetAngularInertiaAndMass(){
		...
		var spar_mass = 0.160;
		var spar_inertia = getInertia(spar_mass, 0.01, 2, 0.015);
		
		var xps_mass = 0.200;
		var xps_inertia = getInertia(xps_mass, 0.2, 2, 0.02);
		
		var cfk_mass = 0.250;
		var cfk_inertia = getInertia(cfk_mass, 0.2, 2, 0.02);
	...
	update(wind, line_tension, timestep_in_s){
		...
		for (let kitePart of this.aerodynamicKiteParts){
			...
			torque.add(forceData.pressure_centre.cross(this.world2kite(forceData.force)));
		}
		...
		torque.add(line_force_dir_in_kite_coords.cross(this.getRelativePositionOfLineKnotInKiteCoordinates()).multiplyScalar(line_tension));
