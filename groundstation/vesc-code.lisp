        ...
        (if (> num-bytes-read 15)
            (if (and (= (bufget-f32 array 0) 1234567.0) (= (bufget-f32 array 12) -1234567.0))
                ...
                (progn
                    ...
                    ;    )
                    ;)
                    (if (= (bufget-f32 array 4) 0)
                        (define speed-pitot (bufget-f32 array 8))
                    )
                )
            )
        )
        ...
        (if (= flightmode launch)
            ...
            (if (= flightmode eight)
                (progn
                    ...
                    (print s)
                    (if (> s 0)
                        (if (and (> s 5) (> line-length 120))
                        	; flightmode = landing or final-landing, induced by kite deciding to return during eight flight or really low wind or difficult turns
                ...
                (if (= flightmode emergency-landing)
	                ...
	                (progn
	                	(define line-length-current 0)
	                	(if (> line-length (+ 15.75 safety-distance-for-landing-debug))
	                	    (define line-length-current 8)
	                	    (if (> line-length safety-distance-for-landing-debug)
	                	        (define line-length-current (+ 1.7 (* 0.4 (- line-length safety-distance-for-landing-debug))))
	                	        (define line-length-current 1.7)
	                	    )
	                	)
	                	(define pitot-current (- 20 (* 1.6 speed-pitot)))
	                	(define line-speed-current (- 8 (* 1.6 (get-speed))))
	                	(define mix pitot-current)
	                	(if (> line-speed-current mix)
	                		(define mix line-speed-current)
	                	)
	                	(if (> line-length-current mix)
	                		(define mix line-length-current)
	                	)
	                	(if (< mix 1.7)
	                		(define mix 1.7)
	                	)
